...
def handle_data():
    ...
    if not received:
        ...
        received = True
        threading.Thread(target=do_qualification(data["mat"])).start()

        return "Данные получены, запускаю основной код"
    return "Сервер уже получил данные ранее"


def do_qualification(mat_string):
    print("Матрица получена, начинаем квалификацию")
    mat_list = string_to_list(mat_string)
    print("матрица:", mat_list)
    qual(mat_list)
