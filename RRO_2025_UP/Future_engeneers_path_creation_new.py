        ...
        for j in range(len(mat[i])):  # для каждого элемента каждого ряда
            ...
            if mat[i][j] == 70:  # если клетка из которой идем - первый этаж
                ...
                if i + 1 < len(mat) and ((mat[i + 1][j] == 70) or (mat[i + 1][j] == 30)):
                    # если при попытке пройти в + по у мы не выйдем за пределы поля и клетка в которую идем - элемент первого этажа
                    # или рампа направленная в нужную сторону, то записываем ее в список
                    # print("success")
                    neighbour_dict[(i, j)].append((i + 1, j))
                    # добавляем в список возможных клеток исходную с измененной координатой
            ...
            # todo uncomment to move freely on sec floor
                ...
                if i + 1 < 8 and ((mat[i + 1][j] == 11 or mat[i + 1][j] == 10) or (mat[i + 1][
                                                                                       j] == 31)):
                    # не выходим за матрицу и клетка в которую хотим пройти - элемент второго этажа или рампа на съезд
...
def wave_frame_displaying(cords, w_num, max_w_num, object, toggle_ramk=1, toggle_mycolor=None, custom_num=0, line_smth=(3, 0.8), mat=[]):
    ...
    if not toggle_mycolor:
        ...
        cv2.putText(object, str(custom_num), ((cords[1] - 1) * 100 + 90 + add, cords[0] * 100 + 90), cv2.FONT_HERSHEY_SIMPLEX, 0.9,
                    (0, 250, 0), 2)  # рисуем текст (числа после плюса подобраны вручную хд)
...
def wave_ini(p_start, connections_dict):
    # построение волны из p_start (Y1,X1)
    waves = [[p_start]]
    ...
    while len(waves[-1]) != 0:  # пока длина последней построенной волны не равна 0
        ...
        for i in range(len(waves[-2])):  # перебираем все точки из предыдущей волны
            ...
            for j in range(len(connections_dict[waves[-2][i]])):
                # для всех точек, в которые можно пройти из выбранной точки предыдущей волны

                if connections_dict[waves[-2][i]][j] not in all_waves:
                    # если точка в которую можно пройти из выбранной точки прошлой волны еще не использована - добавляем ее в текущую волну
                    # ps -2 - индекс прошлой волны, i - индекс конкретного элемента этой волны. Это ключ к элементу словаря,
                    # который содержит все клетки в которые можно пройти. j - индекс конкретной клетки в которую можно пройти
...
def wave_back_way(waves, p1, p2, dict, length_debugging, object, field_mat, max_length=None, ):
    # waves - список волн получаемый из wave_ini
    # p1 , p2 - точка старта(совпадающая со стартовой в wave_ini), и точка финиша
    # dict - из neigbour_ini
    # object - frame
    ...
    while 1:
        ...
        min_num = 100  # очень надеюсь что будет меньше 100 волн...
        ...
        if max_length:  # аргумент из штук с рампами, чтобы не строить маршрут если он уже длиннее минимального
            if len(way) + final_app >= max_length:
                return None
        ...
        if way[-1] == p1:  # если пришли в исходную точку (маршрут построен)
            ...
            if final_app != 0 and smth_for_ramps == False:  # никакой рекурсии!)
                ...
                if length_debugging == 2:
                    print("Seems like theres ramps on our way... \n Lets try to reconstruct it!")
                    print("current lenght:", len(way) + final_app)

                smth_for_ramps = True
                way_with_ramps = ramp_security_control(imposters, length_debugging, len(way) + final_app, dict, object, p1, p2,
                                                       field_mat)  # вызываем функцию ре-маршрутизации
...
def ramp_security_control(ramps, debugging, current_len, dict, object, p1, p2, field_mat):
    ...
    ramps_combinations = all_combinations(
        ramps)  # все комбинации из рамп маршрута, вообще можно было бы сделать комбинацию из всех рамп на поле, но питон не вывозит
    for i in range(len(ramps_combinations)):  # для всех комбинаций рамп
        ...
        if way:  # если путь существует
            ...
            all_path_dict[way[1] + final_app] = way[
                0]  # добавляем путь в словарь всех путей (пофигу что пути с одинаковой длиной будут перезаписываться)
...
def way_visualisation(object, way, funny_sound, anim, color=(0, 255, 0), thickness=10, delta=0, field_mat=[]):
    if way:
        for i in range(len(way) - 1):
            cv2.line(object, (way[i][1] * 100 + 30 + delta * 10, way[i][0] * 100 + 30 + delta * 10),
                     (way[i + 1][1] * 100 + 30 + delta * 10, way[i + 1][0] * 100 + 30 + delta * 10), color, thickness)
            cv2.imshow("map", cv2.resize(object, (600, 600)))
            cv2.waitKey(1)
...
def all_permutations(elements):
    ...
    return tuples_to_lists(permutations)
        ...
        for j in range(len(field_mat)):
            ...
            if list(str(field_mat[i][j]))[-2] == "4":  # если клетка - труба
                ...
                if list(str(field_mat[i][j]))[-1] == "1":  # смотрим куда повернута труба
                    ...
                    if j - 1 > -1 and str(field_mat[i][j])[0] == str(field_mat[i][j - 1])[0] and (
                    i, j - 1) in waves_all:  # если на том же этаже и есть в волнах, то считаем что с этой клетки можно забрать
                        cell_to_tube.append((i, j - 1))
...
def final_roadmap(obj, field_mat, ramp_checkment=False, skip_all_cv=False):
    ...
    real_all_paths_no_joke = {}  # all possible tubes picking ways, storing like [path length] = [ways of path list]
    way_lengths = []  # war to store all ways length (keys to real_all_paths_no_joke)
    ...
    cells_to_tubes_dict[unload_cords[0]] = [unload_cells]  # добавление точек разгрузки труб
    ...
    for i in range(len(tubes)):  # для всех вариантов сбора
        ...
        for j in range(len(tubes[i])):  # для всех труб в вариантах сбора
            ...
            for q in range(len(cells_to_tubes_dict[tubes[i][j]][0])):
                # для всех клеток с которых можно забрать конкретную трубу
                ...
                way = wave_back_way(waves, p1_tmp, cells_to_tubes_dict[(tubes[i][j])][0][q], dictionary, 0, obj,
                                    field_mat)  # маршрут к одной из клеток из которой можно забрать трубу
                ...
                if way:  # если путь есть, то записываем его в словарь с путями
                    ...
                    print("Failed while creating one of final ways. \n Seems like field was scanned incorrectly", "\n p1:", p1_tmp, "p2:",
                          cells_to_tubes_dict[(tubes[i][j])][0][q])
            ...
            way_length += min(ways_to_tube)
            p1_tmp = ways_to_tube_dict[min(ways_to_tube)][-1]

        real_all_paths_no_joke[way_length] = path_to_all  # как только закончился цикл - записываем получившийся маршрут
        way_lengths.append(way_length)  # и его длину (я не умею работать с ключами словаря(()
        ...
        if skip_all_cv:
            for help_me in range(len(real_all_paths_no_joke[min(way_lengths)])):  # рисуем один из возможных маршрутов
    ...
    if skip_all_cv:
        ...
        for g in range(len(real_all_paths_no_joke[min(way_lengths)])):
            way_visualisation(obj, real_all_paths_no_joke[min(way_lengths)][g], 0, 0, get_colors(g), 10, g, field_mat)
            wave_frame_displaying(real_all_paths_no_joke[min(way_lengths)][g][-1], 0, 0, obj, 0, 1,
                                  g + 1)  # рисуем номер в клетке с которой забираем трубу
            cv2.imshow("map", cv2.resize(obj, (600, 600)))
            cv2.waitKey(0)

        for g in range(len(real_all_paths_no_joke[min(way_lengths)])):
            way_visualisation(obj, real_all_paths_no_joke[min(way_lengths)][g], 0, 1, get_colors(g), 10, g, field_mat)
            wave_frame_displaying(real_all_paths_no_joke[min(way_lengths)][g][-1], 0, 0, obj, 0, 1,
                                  g + 1)  # рисуем номер в клетке с которой забираем трубу
            cv2.imshow("map", cv2.resize(obj, (600, 600)))
            cv2.waitKey(1)
        ...
        way_lengths = remove_duplicates(way_lengths)  # для красивого вывода

        print_colored("\n\n\npossible paths lenghts:" + "  " + str([i for i in way_lengths]), "blue")
        print_colored("our choice:" + "  " + str(min(way_lengths)), "blue")

        print_colored("\n\n\n\n\n\n\n\n      WAY DONE", "green")

    return real_all_paths_no_joke[min(way_lengths)]
...
def create_path(mat, enable_visual=0):
    unload_dict = {"l": ["P1", "R1", "X1", "L1", "P1", "R1", "X1", "L1", "P1"], "r": ["P1", "L1", "X1", "R1", "P1", "L1", "X1", "R1", "P1"],
